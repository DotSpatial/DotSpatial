...
namespace DotSpatial.Positioning.Forms
{
    ...
    public abstract class DoubleBufferedControl : Control
    {
            ...
            set
            {
                ...
                if (!value)
                {
                    ...
                    if (
                        ...
                        _paintingThread != null
                        // If the thread is dead, skip
#if PocketPC
                        && _IsPaintingThreadAlive
#else
                        && _paintingThread.IsAlive
#endif
)
                    {
                        // Abort the thread
                        _paintingThread.Abort();
                    }
                }
                else
                {
                    ...
                    _paintingThread = new Thread(PaintingThreadProc)
                                          {
...
#else
                        _offScreenBitmap = new Bitmap(MyWidth, MyHeight); //, PixelFormat.Format32bppPArgb);
#endif

                        // Create new graphics for the bitmap
                        OffScreenGraphics = Graphics.FromImage(_offScreenBitmap);

#if !PocketPC
                        // Apply graphics quality settings
                        _graphicsSettings.Apply(OffScreenGraphics);
#endif
                    }
            ...
            try
            {
                ...
                if (_isExceptionTextAllowed)
                {
                    ...
                    lock (_offScreenSyncRoot)
                    {
                        ...
                        OffScreenGraphics.Clear(SystemColors.Control);
#if PocketPC
...
#else
                        OffScreenGraphics.DrawString(exception.ToString(), Font, SystemBrushes.ControlText, ClientRectangle);
#endif
        ...
        public new int Width
        {
            ...
            set
            {
                if (MyWidth.Equals(value)) return;
                MyWidth = value;
                base.Width = value;
            }
        }
        ...
        public new Size Size
        {
            ...
            set
            {
                if (MyWidth.Equals(value.Width) && MyHeight.Equals(value.Height))
                    return;
        ...
        public new int Height
        {
            ...
            set
            {
                if (MyHeight.Equals(value)) return;
                MyHeight = value;
                base.Height = value;
            }
        }
        ...
        private void PaintingThreadProc()
        {
            ...
            while (_isPaintingOnSeparateThread)
            {
                try
                {
                    ...
                    // Wait for rendering
                    _renderRequestWaitHandle.WaitOne();
 ...
 && _paintingThread.IsAlive
        ...
        public Point Center
        {
            get
            {
                return new Point((int)(MyWidth * 0.5), (int)(MyHeight * 0.5));
            }
        }
