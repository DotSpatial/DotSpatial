...
namespace DotSpatial.Symbology.Forms
{
    ...
    public partial class SQLQueryControl : UserControl
    {
        ...
        private IDataTable _table; // CGX AERO GLZ
        ...
        private void lbxFields_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (_attributeSource != null)
            {
                ...
                for (int page = 0; page < numPages; page++)
                {
                    ...
                    if (table is DS_DataTable)
                        GetMinMax(table.Rows.Cast<DataRow>(), field, ref min, ref max); // CGX AERO GLZ
                    else
                        GetMinMax(table.Rows.Cast<IDataRow>(), field, ref min, ref max); // CGX AERO GLZ
                }
            }
            if (_table != null)
            {
                if (_table is DS_DataTable)
                    GetMinMax(_table.Rows.Cast<DataRow>(), field, ref min, ref max); // CGX AERO GLZ
                else
                    GetMinMax(_table.Rows.Cast<IDataRow>(), field, ref min, ref max); // CGX AERO GLZ
            }
        ...
        private static void GetMinMax(IEnumerable<IDataRow> rows, string field, ref IComparable min, ref IComparable max) // CGX AERO GLZ
        {
            foreach (var dr in rows)
            {
                if ((dr[field] is DBNull)) continue;
        ...
        private void btnGetUniqueValues_Click(object sender, EventArgs e)
        {
            ...
            if (_attributeSource != null)
            {
                ...
                for (int page = 0; page < numPages; page++)
                {
                    IDataTable table = _attributeSource.GetAttributes(page * 10000, 10000); // CGX AERO GLZ
                    foreach (IDataRow dr in table.Rows) // CGX AERO GLZ
                    {
            ...
            else if (_table != null)
            {
                ...
                if (_table is DS_DataTable)
                {
                    foreach (DataRow dr in _table.Rows) // CGX AERO GLZ
                    {
                        if (dr[fieldName] is DBNull) continue;
                        if (lst.Contains(dr[fieldName])) continue;
                        lst.Add(dr[fieldName]);
                ...
                {
                    foreach (IDataRow dr in _table.Rows) // CGX AERO GLZ
                    {
                        if (dr[fieldName] is DBNull) continue;
                        if (lst.Contains(dr[fieldName])) continue;
                        lst.Add(dr[fieldName]);
