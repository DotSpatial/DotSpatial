...
namespace DotSpatial.Symbology.Forms
{
    ...
    /// </summary>
    public class TableEditorControl2 : UserControl
    {
        #region Events
        ...
        public TableEditorControl2(IFeatureLayer layer)
        {
            FeatureLayer = layer;
            Configure();
            layer.SelectionChanged += SelectedFeaturesChanged;
        }
        ...
        private void ParentFormClosing(object sender, CancelEventArgs e)
        {
            ...
            if (_fidField != null && _featureLayer != null && _featureLayer.DataSet != null)
            {
        ...
        public void ZoomToEditedRow()
        {
            ...
            IDataRow drv = dataGridView1.CurrentRow.DataBoundItem as IDataRow; // CGX AERO GLZ
            if (drv == null) return;
        ...
        private void FeatureLayerSetup()
        {
            ...
            if (_featureLayer.DataSet.AttributesPopulated)
            {
                ...
                dataGridView1.DataSource = _featureLayer.DataSet.DataTable.Table; // CGX SLM : deplacement car sinon on a pas les nouvellesq colomnes (FID)
            }
            else
            {
        ...
        private void AddFid(IDataTable table) // CGX AERO GLZ
        {
            ...
            if (_featureLayer.DataSet != null)
            {
                if (_featureLayer.DataSet.AttributesPopulated)
                {
                    ...
                    if (_featureLayer != null)
                    {
                        ...
                        _featureLayer.IsSelected = !_featureLayer.IsSelected;
                        _featureLayer.IsSelected = !_featureLayer.IsSelected;
                    }
                    _ignoreSelectionChanged = false;
                }
        ...
        private void renameFieldToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //collect the field
            List<string> field = new List<string>();
            IDataTable dt = _featureLayer.DataSet.DataTable; // CGX AERO GLZ
            if (dt == null) return;
            ...
            if (_showOnlySelectedRows)
            {
                if (_featureLayer != null)
                {
                    if (_featureLayer.DataSet.AttributesPopulated)
                    {
                        ...
                        {
                            row.Selected = true;
                            fids.Add(Convert.ToInt32(row.Cells[_fidField].Value));
                        }
            ...
            if (_featureLayer != null)
            {
                if (_featureLayer.DataSet.AttributesPopulated)
                {
                    ...
                    foreach (DataGridViewRow row in dataGridView1.Rows)
                    {
                        ...
                        if (sel)
                        {
                            adds.Add(Convert.ToInt32(row.Cells[_fidField].Value));
                        }
                        else
                        {
                            removes.Add(Convert.ToInt32(row.Cells[_fidField].Value));
                        }
                    }
        ...
        private string BuildFindExpression(string findString)
        {
            ...
            //CGX
            foreach (/*Field*/ DataColumn col in columns)
            {
        ...
        private void CopyFid(string field, bool isNewField)
        {
            ...
            {
                //assign the values
                IDataRow r = _featureLayer.DataSet.DataTable.Rows[i]; // CGX AERO GLZ
