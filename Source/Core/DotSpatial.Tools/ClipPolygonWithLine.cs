...
namespace DotSpatial.Tools
{
    ...
    public class ClipPolygonWithLine : Tool
    {
        ...
        public static bool AccurateClipPolygonWithLine(ref IFeature polygon, ref IFeature line, ref IFeatureSet resultFeatureSet)
        {
            ...
            {
                Point currPt = new(line.Geometry.Coordinates[i]);
        ...
        public static bool ClipMultiPartPolyWithLine(ref IFeature polygon, ref IFeature line, ref IFeatureSet resultFile, bool speedOptimized)
        {
            ...
            if (numParts > 1)
            {
                ...
                SeparateParts(ref polygon, out IFeature[] polyParts);
                ...
                for (int i = 0; i <= numParts - 1; i++)
                {
                    ...
                    if (!NetTopologySuite.Algorithm.Orientation.IsCCW(currPart.Geometry.Coordinates))
                    {
                        if (speedOptimized)
                        {
        ...
        public static void CombineParts(ref IFeature[] parts, out IFeature resultShp)
        {
            ...
            Polygon po = new(new LinearRing(parts[0].Geometry.Coordinates));
            for (int i = 0; i <= numParts - 1; i++)
            {
        ...
        public static int FindIntersections(IFeatureSet lineFeatureSet, IFeature polygon, out int[] intersectsPerLineSeg, out Point[][] intersectionPts, out int[][] polyIntersectLocs)
        {
                    ...
                    switch (vertNo)
                    {
                        ...
                        default:
                            ...
                            if (detSigns[lineNo][vertNo - 1] != detSigns[lineNo][vertNo])
                            {
                                ...
                                LineString polyTestLine1 = new(secCoor.ToArray());
                                secCoor = new List<Coordinate>
                                              {
                                                  coorLine[0],
                                                  coorLine[1]
                                              };
                                LineString polyTestLine2 = new(secCoor.ToArray());
                                bool validIntersect = polyTestLine1.Intersects(polyTestLine2);
                                Geometry inPt = polyTestLine1.Intersection(polyTestLine2);
        ...
        public static void FixMultiPartPoly(ref IFeature polygon)
        {
            ...
            SeparateParts(ref polygon, out IFeature[] parts);
            for (int i = 0; i <= numParts - 1; i++)
            {
                bool currIsClockwise = !NetTopologySuite.Algorithm.Orientation.IsCCW(parts[i].Geometry.Coordinates);
                ...
                for (int j = 0; j <= numParts - 1; j++)
                {
                    ...
                    Point pt = new(parts[i].Geometry.Coordinates[0]);
            ...
            CombineParts(ref parts, out IFeature resultShp);
            polygon = resultShp;
        ...
        public bool Execute(IFeatureSet input1, IFeatureSet input2, IFeatureSet output, ICancelProgressHandler cancelProgressHandler)
        {
            ...
            {
                Polygon poly = new(new LinearRing(output.Features[i].Geometry.Coordinates));
        ...
        private static bool FastProcessPartInAndOut(bool[] insidePts, IFeature line, IFeature polygon, IFeatureSet resultFeatureSet)
        {
            ...
            int numIntersects = FindIntersections(lineSegments, polygon, out int[] intersectsPerSeg, out intersectPts, out polyIntLocs);
            ...
            List<Coordinate> insideLineList = new();

            Point startIntersect = Point.Empty;
        ...
        private static bool ProcessAllInside(ref IFeature line, ref IFeature polygon, ref IFeatureSet resultFeatureSet)
        {
            ...
            if (FindIntersections(lineSegments, polygon, out int[] intersectsPerSeg, out intersectPts, out polyIntLocs) == 0) return true; // entire line is inside the polygon, no splitting occurs

            // intersections exist! Find out where.
            List<Coordinate> intersectSegList = new();
            for (int i = 0; i <= numLineSegs - 1; i++)
            {
                int numSegIntersects = intersectsPerSeg[i];
                ...
                else
                {
                    ...
                    Point startPt = new(lineSegments.Features[i].Geometry.Coordinates[0]);
        ...
        private static bool ProcessAllOutside(ref IFeature line, ref IFeature polygon, ref IFeatureSet resultFeatureSet)
        {
            ...
            int numOriginalIntersects = FindIntersections(lineSegments, polygon, out int[] intersectsPerSeg, out intersectPts, out polyIntLocs);
        ...
        private static bool ProcessPartInAndOut(ref bool[] insidePts, ref IFeature line, ref IFeature polygon, ref IFeatureSet resultFeatureSet)
        {
            ...
            List<IFeature> colLineParts = new();
            List<Coordinate> colPolyIntLocPoints = new();
        ...
        private static bool ProcessPartInAndOutWithWholeLine(ref bool[] insidePts, ref IFeature line, ref IFeature polygon, ref IFeatureSet resultFeatureSet, int numIntersects, int numLineSegs, Point[][] intersectPts, int[][] polyIntLocs, int numPolyPts, List<IFeature> colLineParts, List<Coordinate> colPolyIntLocPoints)
        {
            Point[] intPts = new Point[numIntersects];
            Point startPt = new(line.Geometry.Coordinates[0]);
            Point firstIntPt = null;
            Point lastIntPt = null;
            List<int> listIntLocs = new();
            List<int> listLinePartIndexValuesForOrigLine = new();
            List<List<int>> colListForEachLinePart = new();
            List<int> listLinePartStartEndIndexValues = new();
            int nNewLineStartIndex = 0;
            int nNewLineEndIndex = 0;
            int nLastIndexUsed = -1;
            List<Coordinate> listNewLine = new();
            int nLineParts = 0;
            ...
            if (colLineParts.Count == 0)
            {
                ...
                for (int i = 0; i < (listIntLocs.Count / 2); i++)
                {
                    ...
                    List<Coordinate> listLinePart = new() { listNewLine[nStart] };
        ...
        private static void RemoveDuplicateFeaturesFromFeatureSet(ref IFeatureSet resultFeatureSet)
        {
            ...
            List<int> listIndexesToNotAdd = new();
            List<int> listIndexesAddedToNewFs = new();
        ...
        private static bool SectionPolygonWithLine(ref IFeature line, ref IFeature polygon, int polyStart, int polyEnd, ref IFeatureSet resultFeatureSet)
        {
            ...
            SplitPolyInTwo(ref line, ref polygon, polyStart, polyEnd, out IFeature poly1, out IFeature poly2);
            if (numResults == 0)
            {
        ...
        private static void SplitPolyInTwo(ref IFeature line, ref IFeature polygon, int beginPolySeg, int endPolySeg, out IFeature poly1, out IFeature poly2)
        {
            ...
            List<Coordinate> firstPartList = new();
            List<Coordinate> secondPartList = new();
            int numPolyPts = polygon.Geometry.NumPoints;
            int numLinePts = line.Geometry.NumPoints;
            int count;
