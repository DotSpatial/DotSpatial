...
namespace DotSpatial.Topology.Utilities
{
    ...
    public class GeometricShapeFactory
    {
        ...
        public IPolygon CeateEllipse()
        {
            ...
            var centreX = env.Minimum.X + xRadius;
            var centreY = env.Minimum.Y + yRadius;
        ...
        public IPolygon CreateRectangle()
        {
            ...
            {
                var x = env.Maximum.X;
                var y = env.Minimum.Y + i * ySegLen;
                pts[ipt++] = CreateCoord(x, y);
            }
            for (i = 0; i < nSide; i++) 
            {
                var x = env.Maximum.X - i * xSegLen;
                var y = env.Maximum.Y;
                pts[ipt++] = CreateCoord(x, y);
            }
            for (i = 0; i < nSide; i++) 
        ...
        protected class Dimensions
        {
            ...
            public Envelope Envelope
            {
                ...
                set
                {
                    ...
                    _base = new Coordinate(value.Minimum.X, value.Minimum.Y);
                    _centre = new Coordinate(value.Center());
                }
            }
